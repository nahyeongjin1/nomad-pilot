...
City 엔티티에 nullable 이미지 관련 컬럼 추가:
...
imageAuthorName!: string | null;  // "John Doe"

@Column({ type: 'varchar', length: 500, nullable: true })
imageAuthorUrl!: string | null;  // "https://unsplash.com/@johndoe"
```

마이그레이션 자동 생성으로 `ALTER TABLE cities ADD COLUMN image_url, image_author_name, image_author_url` 적용. 프론트에서 UTM 파라미터 포함 attribution 링크를 조립하여 렌더링.
...
- 응답에서 추출:
  ...
  - `results[0].user.name` → authorName
  - `results[0].user.links.html` → authorUrl
...
UPDATE cities SET image_url = 'https://images.unsplash.com/photo-xxx?w=800',
                  image_author_name = 'John Doe',
                  image_author_url = 'https://unsplash.com/@johndoe'
WHERE name_en = 'Tokyo';
-- ... 6개 도시
```
...
| 파일                                                              | 작업                                                          |
| ----------------------------------------------------------------- | ------------------------------------------------------------- |
| `apps/backend/src/cities/entities/city.entity.ts`                 | `imageUrl`, `imageAuthorName`, `imageAuthorUrl` 컬럼 추가     |
...
| 마이그레이션 2건                                                  | 이미지 컬럼(imageUrl, imageAuthorName, imageAuthorUrl) + 시드 |
| 테스트 파일들                                                     | 단위 + E2E                                                    |

## 주의사항

- Travelpayouts Data API 토큰(`TRAVELPAYOUTS_API_TOKEN`)은 기존 마커(`TRAVELPAYOUTS_MARKER`)와 별도. 대시보드에서 API 토큰 확인 필요
- Unsplash 이미지 사용 시 API 정책 준수 필수: attribution + 다운로드 추적. `imageAuthorName` + `imageAuthorUrl`을 구조화 데이터로 저장, 프론트에서 UTM 파라미터 포함 링크 조립
...
- Travelpayouts/Amadeus API 호출 시 에러 로그에 요청 URL을 그대로 출력하면 쿼리스트링의 API 토큰이 노출됨. 에러 로깅 시 URL 로깅 금지 또는 토큰 마스킹 처리 필수
- 외부 API(Travelpayouts, Amadeus) 호출 시 HttpService에 timeout 설정 필수 (10초). 응답 지연 시 무한 대기 방지
