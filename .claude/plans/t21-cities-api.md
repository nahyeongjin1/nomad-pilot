...
- **Unsplash API 정책 준수:**
  - **Attribution 링크:** imageCredit에 작가 프로필 링크 포함 — `Photo by <a href="{authorUrl}?utm_source=nomad_pilot&utm_medium=referral">{authorName}</a> on <a href="https://unsplash.com?utm_source=nomad_pilot&utm_medium=referral">Unsplash</a>` 형태로 저장
...
- Unsplash 이미지 사용 시 API 정책 준수 필수: attribution 링크(UTM 파라미터 포함) + 다운로드 추적 호출. `imageCredit`에 HTML 링크 형태로 저장, 프론트에서 렌더링
