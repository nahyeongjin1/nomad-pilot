...
> 2026-02-26 실행. 환경: Docker PostGIS 17-3.5, WSL2
...
데이터 소스: Overpass API (OSM). 도쿄/오사카는 목표(10K)를 크게 초과하여 스트레스 테스트 역할 수행.

### 결과 요약

| 쿼리             | 조건                   | 결과 행 | Execution (warm) | 목표   | 판정 |
| ---------------- | ---------------------- | ------- | ---------------- | ------ | ---- |
| Q1 반경 검색     | 도쿄역 1km             | 958     | **19ms**         | < 20ms | Pass |
| Q2 반경+필터     | 도쿄역 1km, restaurant | 379     | **15ms**         | < 20ms | Pass |
| Q3 거리 매트릭스 | 20개 POI 쌍            | 190     | **36ms**         | < 50ms | Pass |
| Q6 주변 추천     | 오사카역 500m, cafe    | 102     | **15ms**         | < 20ms | Pass |

### 스트레스 테스트 (추가)

| 조건                    | 결과 행 | Execution | 비고                      |
| ----------------------- | ------- | --------- | ------------------------- |
| 신주쿠 2km, 필터 없음   | 2,597   | **262ms** | geography 거리 계산 비용  |
| 신주쿠 2km + restaurant | 1,122   | **26ms**  | 카테고리 필터로 극적 개선 |

### 실행 계획 분석

- **Q1/Q2/Q6**: GiST Index Scan → Bitmap Heap Scan → Filter. 인덱스 정상 활용
- **Q3**: PK Index (UUID) → Nested Loop + Materialize. 20개 POI 조합이라 GiST 불필요
- 2km 무필터 시 `ST_DWithin` + `ST_Distance` 연산이 2,500+ 행에 적용되어 병목
- category 필터 추가 시 `BitmapAnd`로 GiST + B-tree(category) + B-tree(city_id) 3개 인덱스 조합

### 판정

| 항목           | 결과       | 비고                                     |
| -------------- | ---------- | ---------------------------------------- |
| 성능 목표 달성 | **Pass**   | 모든 대상 쿼리(Q1/Q2/Q3/Q6) 목표 이내    |
| geography 유지 | **유지**   | 1km 이내 20ms 달성. geometry 전환 불필요 |
| 추가 인덱스    | **불필요** | 단일 GiST + 기존 B-tree 조합으로 충분    |

### 권고 사항

- 2km 이상 반경 검색 시 **반드시 category 필터 포함** (서비스 레이어에서 강제)
- 도시당 POI 규모가 10만 건 이상으로 증가 시 geography → geometry 전환 재검토
- 현재 인덱스 구성으로 MVP 충분. 복합 인덱스는 프로덕션 쿼리 패턴 확인 후 검토
